...
2. ツールバーの **Dopamine Train のアイコンをクリック** → トレーナーウィンドウが
   別ウィンドウ（560×900）で開く
3. カメラ許可を求められたら **許可**
4. キャリブレーション: Mac を体の**斜め45度**（**左右どちらでもよい**）に置いて
   **1.5〜2m** 離れる → 「開始」→ 腕を伸ばし切って1秒 →
   「腕を巻き上げて記録開始」→ 巻き上げて1秒
   - **カメラに近い側の腕**でカールする。どちらの腕かは自動判定される（下の
     「キャリブレーションの受理条件」参照）
   - 完了画面に採用された腕・ROM・信頼度・推定した向きが出るので、意図通りか確認できる
...
`resolveCalibration()`（[calibration.ts](src/core/detect/calibration.ts)）が唯一の判定箇所。
下端・上端の各1秒で**両腕を同時に観測**し、次の3つを満たす候補のうち **ROM が最大の腕**を
採用する。

| ゲート | 条件 |
|---|---|
| 信頼度 | 肩・肘・手首の score の **p10** ≥ 0.5（記録フレーム ≥ 3） |
| ROM | `elbow-angle`: \|上端−下端\| ≥ 60° ／ `wrist-height`: ≥ 上腕長0.8本 |
| 向き | `elbow-angle`: 下端 > 上端 ／ `wrist-height`: 上端 > 下端 |

**腕の左右とカメラの向きは宣言しない。** カメラに近い腕はよく見えて可動域も大きく写り、
奥の腕は体に隠れて信頼度が落ちるか、動かしていなければ ROM が出ない — 結果として
「カメラに近い側の、実際に動かした腕」が自動的に選ばれる。これで**カメラを体の左右
どちらの斜め45度に置いても同じ手順で使える**。

体の向きは `肩幅 / 上腕長` から推定して `Calibration.view` に記録する（正面で比が大きく、
真横で0に近づく。上腕長で割るのでカメラからの距離に対してスケール不変）。ただし
**この推定で受理を拒否しない** — しきい値（1.1 / 0.35）が実機未検証なので、ずれていたら
正しく45度に立っているのに永久に通らなくなる。正面寄りと判定されたときに
「肘の角度は正面だと潰れる」と警告するだけにして、実際のゲートは物理的に意味のある
ROM と信頼度に任せている。

失敗時はウィザードに**両腕の実測値**（ROM / 信頼度 p10 / 各ゲートの ✓✗ / 推定した向き /
上腕長px / 姿勢を取れなかったフレーム数）を表示する。理由の文言だけだと信頼度が 0.48 なのか
0.05 なのか分からず、照明・距離・角度のどれを直せばいいか判断できない。

#### 過去に踏んだ罠（同じ轍を踏まないための記録）

- **`score` の最小値で判定していた** → 実質不合格の強制だった。1秒×24fps に対して
  「全フレーム × 3キーポイントすべてが閾値以上」を要求することになり、1フレームでも
  手首の信頼度が落ちれば全部やり直し。1フレームの合格率が95%でも
  `0.95²⁴ ≒ 29%` しか通らない（記録時間を延ばすと失敗率が上がる逆向きの性質）。
  → **p10 に変更**。raw 値に p10/p50/p90 を使う設計判断が score だけに適用されて
  いなかった、という一貫性の欠落でもあった。
- **姿勢が取れなかったフレームを `score:0` のダミーとして記録バッファに入れていた**
  → 1フレーム落ちるだけで `minScore = 0` になり `low_confidence` 確定。
  ダミーサンプル自体は detector の `lost` 判定を一元化するために必要だが、
  キャリブレーションの代表値に混ぜてはいけない（欠測は観測値ではない）。
  → `pose-source.ts` で観測中は**姿勢が取れたフレームのみ**バッファに入れ、
  落ちた数は `droppedFrames` で別に数えて診断表示する。
- **`CameraView` が保存されるだけで一度も読まれていなかった**（デッドメタデータ）。
  カメラをどこに置いても検証が走らず、ウィザードにも位置の案内が無かった。
  → 宣言をやめて実測に変更。ウィザードに位置・距離の指示を明記。
...
- [x] キャリブレーションウィザードの UI フロー（開始→下端記録→上端記録→結果表示）
      と、失敗時のエラーメッセージ・診断表・「最初からやり直す」導線を Playwright で
      自動検証（偽カメラ映像には人物が映らないため `no_frames` で正しく失敗し、
      診断表に両腕とも ✗、「姿勢を取れなかったフレーム: 28」が出ることを確認）
- [x] 位置・距離の案内（斜め45度・左右どちらでも・1.5〜2m）が intro に出る
- [ ] **カメラを体の左（右）斜め45度に置いて、左（右）腕が自動採用される**
      ※ロジックは calibration.test.ts で両方向を検証済みだが、実機の
      「奥の腕の信頼度が本当に落ちるか」は実測が必要
- [ ] 完了画面の実測値（ROM / 信頼度 p10 / 肩幅比 / 上腕長px）が妥当な範囲か。
      特に**肩幅比の実測値**を見て `SHOULDER_RATIO_FRONT` / `_SIDE`（1.1 / 0.35、
      現在は未検証の見積り）を詰める
